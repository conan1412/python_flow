...
def sigmoid(q):  # 数字越往正无穷靠近，数据越容易溢出爆炸
    q = np.clip(q, -100, 10000000)  # 解决sigmoid数据溢出
    return 1 / (1 + np.exp(-q))

def softmax(x):
    ''' x.shape: (200, 10), np.max(x, axis=1, keepdims=True).shape: (200, 1)
    解决softmax数据溢出，防止x过大导致np.exp(x)数据爆炸溢出，
    并且这种减去最大值的方式是一种平移机制，基本不影响每条数据之间的差异性 '''
    x = x - np.max(x, axis=1, keepdims=True)
    ex = np.exp(x)
    sum_ex = np.sum(ex,axis=1,keepdims=True)  # 多维度矩阵必须要keepdims=True
    result = ex / sum_ex
    result = np.clip(result, 1e-100, 1)  # 解决loss数据溢出，防止最终输出的x过小，导致log(x)数据爆炸溢出
    return result
